...
def build_jobs(args) -> List[Dict[str, object]]:
    ...
    specs = [
        ("late_fusion_v1_ce", "v1", "ce"),
        ("late_fusion_v2_ce", "v2", "ce"),
        ("late_fusion_v1_focal", "v1", "focal"),
        ("late_fusion_v2_focal", "v2", "focal"),
    ]
...
def write_shell(jobs: List[Dict[str, object]], output_root: Path) -> Path:
    ...
    for job in jobs:
        ...
        lines.append("")
    shell_script_path = output_root / "run.sh"
    shell_script_path.write_text("\n".join(lines), encoding="utf-8")
    return shell_script_path


def main() -> None:
    parser = argparse.ArgumentParser(description="Generate the round-2 late-fusion experiment plan.")
    ...
    parser.add_argument("--output-root", type=Path, default=Path("experiment_runs/late_fusion_round2"))
    ...
    shell_script_path = write_shell(jobs, args.output_root)
